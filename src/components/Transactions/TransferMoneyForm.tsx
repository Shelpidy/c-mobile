...
const TransferMoneyForm = ({ navigation }: TransferMoneyFormProps) => {
   ...
   const [transfereeAccountNumber,setTransfereeAccountNumber] = useState<string>("")
   const [currentUser,setCurrentUser] = useState<CurrentUser>({})
   const [amount,setAmount] = useState<string>('')
   const [loading,setLoading] = useState<boolean>(false)

   useEffect(()=>{
    setCurrentUser({id:1,accountNumber:'1COM1000000000',email:"teax@gmail.com"})
      ...
      let postObj = {transfereeAccountNumber,transferorAccontNumber:currentUser.accountNumber,amount}
      try{
        let response = await axios.post("http://192.168.193.183:5000/api/transactions/sendcommodity",postObj)
