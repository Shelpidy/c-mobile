      ...
      let fetchData = async () => {
         ...
         try {
            let response = await fetch(
               `http://192.168.148.183:5000/api/auth/users/${notification.notificationFrom}`,
               { method: "GET" }
            );
   ...
   const handleNotification = async () => {
      ...
      try {
         let { data } = await axios.get(
            `http://192.168.148.183:5000/api/notifications/read/${notId}`
         );
         if (data.status == "success") {
            navigation.navigate("UserProfileScreen", {
               userId: notification.notificationFrom,
            });
            Alert.alert("Failed", data.message);
         }
      } catch (err) {
   ...
   return (
      <Pressable
         ...
         <View>
            <Image
               resizeMode="cover"
               source={{ uri: notFrom.profileImage }}
               style={{
                  width: 50,
                  height: 50,
                  borderRadius: 30,
