import {
 ...
 const FullSharedPostComponent = ({ navigation, route }: FullSharedPostComponentpost) => {
   ...
   useEffect(function () {
    ...
    let fetchData = async () => {
       let fromId = route.params.fromId;
       try {
          let response = await fetch(
             `http://192.168.144.183:5000/api/auth/users/${fromId}`,
             { method: "GET" }
          );
